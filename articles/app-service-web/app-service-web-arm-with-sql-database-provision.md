<properties 
    pageTitle="佈建使用 SQL 資料庫的 web 應用程式" 
    description="若要部署包含 SQL 資料庫的 web 應用程式使用 Azure 資源管理員範本。" 
...
# <a name="provision-a-web-app-with-a-sql-database"></a>佈建與 SQL 資料庫的 web 應用程式

本主題中，您將學習如何建立 Azure 資源管理員範本部署的 web 應用程式和 SQL 資料庫。 您將學習如何定義部署的資源，以及如何定義的參數會指定部署執行時。 您可以使用您自己的部署，這個範本，或自訂以符合您的需求。

如需有關如何建立範本的詳細資訊，請參閱[撰寫 Azure 資源管理員範本](../resource-group-authoring-templates.md)。

如需有關部署應用程式的詳細資訊，請參閱[部署 Azure 中預測複雜的應用程式](app-service-deploy-complex-application-predictably.md)。

完成的範本，請參閱[Web 應用程式與 SQL 資料庫] 範本](https://github.com/Azure/azure-quickstart-templates/blob/master/201-web-app-sql-database/azuredeploy.json)。
...
此範本包含建構範本中使用名稱的變數。 變數值會使用**uniqueString**函數從 [資源群組識別碼產生的名稱。
...
### <a name="sql-server-and-database"></a>SQL Server 與資料庫

建立新的 SQL Server 和資料庫。 指定伺服器的名稱**伺服器名稱**參數和**serverLocation**參數中所指定的位置。 在建立新的伺服器時，您必須提供的登入名稱和密碼，資料庫伺服器管理員。 
...
### <a name="alert-rules-for-status-codes-403-and-500s-high-cpu-and-http-queue-length"></a>狀態碼 403 和 500's 高 CPU 和 HTTP 佇列長度的通知規則 
...
### <a name="app-insights"></a>應用程式的深入見解
