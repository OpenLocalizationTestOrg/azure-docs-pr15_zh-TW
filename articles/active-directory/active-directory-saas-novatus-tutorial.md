<properties
    ...
    description="瞭解如何設定單一登入 Azure Active Directory 和 Novatus 之間。"
...
# <a name="tutorial-azure-active-directory-integration-with-novatus"></a>教學課程︰ 使用 Novatus 的 Azure Active Directory 整合

本教學課程中的目標是以顯示您如何將整合 Novatus 與 Azure Active Directory (Azure AD)。

Azure AD 與整合 Novatus 為您提供下列優點︰

- 您可以控制可存取 Novatus Azure AD
- 您可以讓使用者能自動取得登入 Novatus （單一登入） 至其 Azure AD 帳戶
...
若要設定 Novatus Azure AD 整合，您需要下列項目︰
...
## <a name="adding-novatus-from-the-gallery"></a>從圖庫新增 Novatus
若要設定 Novatus 整合 Azure AD，您需要從庫新增 Novatus 到受管理的 SaaS 應用程式的清單。

**若要從圖庫新增 Novatus，請執行下列步驟︰**
...
6. 在 [搜尋] 方塊中，輸入**Novatus**。
 
    ![建立 Azure AD 測試使用者](./media/active-directory-saas-novatus-tutorial/tutorial_novatus_01.png)

7. 在 [結果] 窗格中，選取**Novatus**，，然後按一下要新增應用程式**完成**。

    ![建立 Azure AD 測試使用者](./media/active-directory-saas-novatus-tutorial/tutorial_novatus_02.png)

##  <a name="configuring-and-testing-azure-ad-single-sign-on"></a>設定及測試 Azure AD 單一登入
本節的目標是以顯示您如何設定和使用 Novatus 根據稱為 「 許承恩 」 的測試使用者測試 Azure AD 單一登入。

單一登入搭配使用，Azure AD 需要知道什麼對應中的使用者 Novatus Azure AD 中的使用者。 也就是說，必須建立連結和之間的關聯 Azure AD 使用者 Novatus 中相關的使用者。

所指派**的使用者名稱**的值為 Novatus**使用者名稱**的值 Azure AD 中建立此連結關聯。

若要設定及測試 Azure AD 單一登入與 Novatus，必須完成下列建置組塊︰

1. **[Azure AD 設定單一登入](#configuring-azure-ad-single-single-sign-on)**-若要讓使用者能使用這項功能。
2. **[建立 Azure AD 測試使用者](#creating-an-azure-ad-test-user)**-Azure AD 單一登入與許承恩測試。
4. **[建立 Novatus 測試使用者](#creating-a-Novatus-test-user)**-連結至 Azure AD 的表示她的 Novatus 中有許承恩的對應。
5. **[指派 Azure AD 測試使用者](#assigning-the-azure-ad-test-user)**-啟用許承恩使用 Azure AD 單一登入。
5. **[測試單一登入](#testing-single-sign-on)**-驗證是否設定運作。

### <a name="configuring-azure-ad-single-sign-on"></a>Azure AD 單一登入設定

本節的目標是 Azure AD 單一登入 Azure 傳統入口網站中啟用並 Novatus 應用程式中設定單一登入。



**若要使用 Novatus 設定 Azure AD 單一登入，請執行下列步驟︰**

1. 在 Azure 的傳統入口網站， **Novatus**應用程式整合在頁面上，按一下 [**設定單一登入**以開啟 [**設定單一登入**] 對話方塊。

    ![設定單一登入][6] 

2. 在**您要如何登入 Novatus 的使用者**] 頁面上，選取**Azure AD 單一登入**，然後按 [**下一步**。
...
3. **設定應用程式設定**] 對話方塊在頁面上，執行下列步驟︰
    ...
    。 在 [登入 URL] 文字方塊中，輸入您登入您的 Novatus 應用程式，使用下列模式的使用者所使用的 URL: **「 https://sso.novatuscontracts.com/companyname 」**。 參照的一般名稱時的**公司名稱**必須要取代的實際的名稱。


4. 在**設定單一登入 Novatus 在**頁面上，執行下列步驟︰
...
5. 若要取得 SSO 應用程式的設定，請連絡 Novatus 支援小組透過jvinci@novatusinc.com。 下載的憑證檔案附加至您的信箱，與他們設定 SSO Novatus 小組共用中繼資料 url （實體識別碼、 SSO 的登入 URL 和登出 URL）。
...
1. 在**Azure 傳統 p ortal**，在左側的功能窗格中，按一下 [ **Active Directory**]。
...
本節的目標是以建立 Novatus 中名為許承恩使用者。 Novatus 支援只時間佈建，這是預設啟用。

沒有動作項目為您在此區段中。 在嘗試存取 Novatus 如果尚不存在，將會建立新的使用者。 [設定 Azure AD 單一登入](#configuring-azure-ad-single-single-sign-on)。

> [AZURE.NOTE] 若要手動建立的使用者，您需要連絡 Novatus 支援小組。


### <a name="assigning-the-azure-ad-test-user"></a>指派 Azure AD 測試使用者

本節的目標是啟用許承恩 Azure 單一登入使用其存取權授與 Novatus。

![為使用者指派][200] 

**若要指定 Novatus 許承恩，請執行下列步驟︰**
...
當您按一下 [Access] 面板中的 [Novatus] 方塊時，您應該取得自動登入 Novatus 應用程式。
