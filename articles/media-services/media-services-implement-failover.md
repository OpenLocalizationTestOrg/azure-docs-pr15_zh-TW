<properties 
    pageTitle="實作串流案例的容錯移轉 |Microsoft Azure" 
    description="本主題說明如何實作串流案例的容錯移轉。" 
...
#<a name="implementing-failover-streaming-scenario"></a>實作串流案例的容錯移轉

此逐步解說從一個資產，到另一個才能處理重複的視串流，示範如何複製內容 (blob)。 這種情況相當實用想要設定其在我們的資料中心的其中一種中斷的情況下兩個資料中心之間的容錯移轉至 CDN 的客戶。
此逐步解說使用 Microsoft Azure 媒體服務 SDK、 Microsoft Azure 媒體服務 REST API 和 Azure 儲存體 SDK，示範下列工作。

1. 設定 「 資料中心的 」 中的媒體服務帳戶。
1. 上傳 mezzanine 檔案，將來源資產。
1. 編碼多位元的率 MP4 檔案的資產。 
1. 建立來源資產相關聯的儲存空間帳戶中有容器的 「 讀取 」 權限的來源資產唯讀 SA 定位器。
1. 從在上一個步驟中建立的唯讀 SA 定位器中取得容器的來源資產名稱。 我們需要此資訊複製 blob 儲存體帳戶 （說明主題稍後的）。
1. 建立資產建立編碼工作的起點定位器。 

然後，處理容錯移轉︰

1. 設定 「 資料中心 B 」 中的媒體服務帳戶。
1. 在目標媒體服務帳戶建立目標空白的資產。
1. 建立目標空白資產目標與目標資產相關聯的儲存空間帳戶中有寫入存取權的容器寫入 SA 定位器。
1. 使用 Azure 儲存體 SDK 來源儲存帳戶 」 資料中心的 」 和 「 資料中心 B 」 中的目標儲存帳戶之間複製二進位大型物件 （資產檔案） （這些儲存帳戶是與感興趣的資產相關聯的）。
1. 建立二進位大型物件 （資產檔案） 複製到目標資產的目標 blob 容器之間的關聯。 
1. 建立 「 資料中心 b 「 資產原點定位器並指定定位器所產生的 「 資料中心的 「 資產的識別碼。 
1. 您會得出串流 Url 位置的 Url 的相對路徑是相同 （僅基底 Url 有所不同）。 
 
然後，處理任何中斷，您可以建立這些原點定位器上方 cdn 到底。 

下列事項︰

- 媒體服務 SDK 的目前版本不支援建立定位器指定的定位器識別碼。 若要完成這項工作，我們將使用媒體服務 REST API。
- 媒體服務 SDK 的目前版本不支援以程式設計方式產生 IAssetFile 資訊會與資產檔案關聯資產。 若要完成這項工作，我們將使用 CreateFileInfos 媒體服務 REST API。 
- 加密的儲存空間資產 (AssetCreationOptions.StorageEncrypted) 不支援複寫 （因為加密金鑰就是兩個媒體服務帳戶的不同）。 
- 如果您想要利用動態包裝，您必須先取得至少有一個視串流保留的單位。 如需詳細資訊，請參閱[動態包裝資產](media-services-dynamic-packaging-overview.md)。
 

>[AZURE.NOTE]請考慮使用另一種手動實作容錯移轉串流案例的媒體服務[複寫器工具](http://replicator.codeplex.com/)。 這項工具可讓您複製跨兩個媒體服務帳戶的資產。

##<a name="prerequisites"></a>必要條件
 
- 新的或現有的 Azure 訂閱中的兩個媒體服務帳戶。 瞭解[如何建立媒體服務帳戶](media-services-portal-create-account.md)。
...
1. 使用 Visual Studio 建立新的方案包含 C# 主控台應用程式專案。 輸入 HandleRedundancyForOnDemandStreaming 名稱，然後按一下 [確定]。
1. 在相同層級 HandleRedundancyForOnDemandStreaming.csproj 專案檔案 SupportFiles 資料夾。 SupportFiles 資料夾下建立 [OutputFiles 和 MP4Files] 資料夾。 將.mp4 檔案複製到 MP4Files 資料夾 （在此範例中，使用 BigBuckBunny.mp4 檔案）。 
...
1. 新增所需的 [此專案的其他參照︰ System.Configuration System.Runtime.Serialization，與 System.Web。
...
##<a name="add-code-that-handles-redundancy-for-on-demand-streaming"></a>新增處理重複的視串流的程式碼。
...
您現在可以使用兩個資料中心，因此若是任何中斷的容錯移轉之間傳送要求流量管理員。
