<properties
   pageTitle="使用.NET SDK 的 HDInsight 中建立 Windows 型 Hadoop 叢集 |Microsoft Azure"
    description="瞭解如何建立 HDInsight 叢集使用.NET SDK Azure HDInsight。"
   services="hdinsight"
   documentationCenter=""
   tags="azure-portal"
   authors="mumian"
...
# <a name="create-windows-based-hadoop-clusters-in-hdinsight-using-net-sdk"></a>使用.NET SDK 的 HDInsight 中建立 Windows 型 Hadoop 叢集

[AZURE.INCLUDE [selector](../../includes/hdinsight-selector-create-clusters.md)]


瞭解如何建立使用.NET SDK HDInsight 叢集。 其他叢集建立的工具和功能按一下此頁面上方的索引標籤上選取，或請參閱[叢集建立方法](hdinsight-provision-clusters.md#cluster-creation-methods)。
...
HDInsight.NET SDK 提供.NET 用戶端文件庫，可讓您更容易使用 HDInsight.NET Framework 應用程式。 按照以下步驟建立 Visual Studio 主控台應用程式，並貼上建立叢集的程式碼。

應用程式需要 Azure 資源] 群組中，與預設儲存的帳戶。  [附錄 A](#appx-a-create-dependent-components)提供的 PowerShell 指令碼，來建立從屬參照的元件。

**若要建立的 Visual Studio 主控台應用程式**

1. 在 Visual Studio 中建立新 C# 主控台應用程式。
2. 執行下列 Nuget 命令 Nuget 封裝管理主控台中檢視。
...
7. 按**F5**執行應用程式。 主控台視窗應該開啟，並顯示應用程式的狀態。 您也會提示您 Azure 帳戶認證。 它可能需要幾分鐘，才能建立 HDInsight 叢集。
...
##<a name="appx-a-create-dependent-components"></a>Appx A 建立相關元件

下列 PowerShell 的 Azure 指令碼可用於建立.NET 應用程式在本教學課程所需的相依元件。
