<properties
    pageTitle="教學課程︰ Azure Active Directory 整合 CS 顆星 |Microsoft Azure"
    description="瞭解如何設定單一登入 Azure Active Directory 和 CS 顆星之間。"
...
# <a name="tutorial-azure-active-directory-integration-with-cs-stars"></a>教學課程︰ 使用 CS 顆星的 Azure Active Directory 整合

本教學課程中的目標是以顯示您如何與 Azure Active Directory (Azure AD) 整合 CS 顆星。  
Azure AD 與整合 CS 顆星為您提供下列優點︰ 

- 您可以控制可存取 CS 顆星 Azure AD 
- 您可以使用其 Azure AD 帳戶啟用使用者會自動取得登入至 CS 顆星 （單一登入）
...
## <a name="adding-cs-stars-from-the-gallery"></a>從圖庫新增 CS 顆星
若要設定 CS 顆星整合 Azure AD，您需要從庫新增 CS 顆星至您的受管理的 SaaS 應用程式清單。

**若要從圖庫新增 CS 顆星，請執行下列步驟︰**
...
7. 在 [結果] 窗格中，選取**CS 顆星**，然後按一下要新增應用程式**完成**。
...
本節的目標是以顯示您如何設定和使用 CS 顆星根據稱為 「 許承恩 」 的測試使用者測試 Azure AD 單一登入。

單一登入搭配使用，Azure AD 需要知道什麼對應中的使用者 CS 顆星至 Azure AD 中的使用者。 也就是說，必須建立連結和之間的關聯 Azure AD 使用者 CS 顆星中相關的使用者。  
所指派**的使用者名稱**的值為 CS 顆星**使用者名稱**的值 Azure AD 中建立此連結關聯。
 
若要設定及測試 Azure AD 單一登入與 CS 顆星，必須完成下列建置組塊︰

1. **[Azure AD 設定單一登入](#configuring-azure-ad-single-single-sign-on)**-若要讓使用者能使用這項功能。
2. **[建立 Azure AD 測試使用者](#creating-an-azure-ad-test-user)**-Azure AD 單一登入與許承恩測試。
4. **[建立 CS 顆星測試使用者](#creating-a-cs-stars-test-user)**-有許承恩的對應中的已連結至她的 Azure AD 表示 CS 顆星。
5. **[指派 Azure AD 測試使用者](#assigning-the-azure-ad-test-user)**-啟用許承恩使用 Azure AD 單一登入。
5. **[測試單一登入](#testing-single-sign-on)**-驗證是否設定運作。

### <a name="configuring-azure-ad-single-sign-on"></a>Azure AD 單一登入設定

本節的目標是 Azure AD 單一登入 Azure 傳統入口網站中啟用並 CS 顆星應用程式中設定單一登入。

**若要使用 CS 顆星設定 Azure AD 單一登入，請執行下列步驟︰**

1. 在 [Azure 傳統入口網站， **CS 顆星**應用程式整合在頁面上，按一下 [**設定單一登入**以開啟 [**設定單一登入**] 對話方塊。

    ![設定單一登入][6]

2. 在**您要如何登入至 CS 顆星的使用者**] 頁面上，選取**Azure AD 單一登入**，然後按 [**下一步**。
...
3. **設定應用程式設定**] 對話方塊在頁面上，執行下列步驟︰ 
    ...
    ![設定應用程式設定][8]
     ...
     3.1 在 [**登入 URL** ] 文字方塊中，輸入您用來 CS 顆星應用程式登入您的使用者的 URL (例如︰ `https://uat.csstars.com/enterprise/default.cmdx?ssoclient=C234UAT2`)。

     > [AZURE.NOTE] 如果您不知道的正確的值，請連絡您沼澤 ClearSight 代表。

     3.2 捨位。 按一下 [**下一步**]。
 
4. 在**設定單一登入 CS 顆星在**頁面上，按一下**下載的中繼資料**]，然後儲存在本機電腦上的中繼資料檔案。

    ![什麼是 Azure AD Connect][9]

5. 若要取得單一登入啟用 CS 顆星，請連絡您沼澤 ClearSight 代表並交出中繼資料檔案。
...
### <a name="creating-a-cs-stars-test-user"></a>建立 CS 顆星測試使用者

本節的目標是以建立 CS 顆星中名為許承恩使用者。

若要取得 CS 顆星中建立的使用者，您需要連絡您的沼澤 ClearSight 代表。


### <a name="assigning-the-azure-ad-test-user"></a>指派 Azure AD 測試使用者

本節的目標是啟用許承恩 Azure 單一登入使用其存取權授與 CS 顆星。

![為使用者指派][200]


**若要指定 CS 顆星許承恩，請執行下列步驟︰**

1. Azure 傳統的入口網站中，若要開啟 [應用程式] 檢視中，在 [目錄] 檢視中，按一下 [**應用程式**頂端的功能表中。

    ![為使用者指派][201]

2. 在 [應用程式] 清單中，選取**CS 顆星**。
...
當您按一下 [Access] 面板中的 [CS 顆星] 方塊時，您應該取得自動登入 CS 顆星應用程式。
