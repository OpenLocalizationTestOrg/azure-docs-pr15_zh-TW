...
在[Azure 入口網站](https://portal.azure.com)，您可以新增通知時通知您的服務的**可用性**低於您指定的臨界值的規則。
...
您也應該檢查是否在用戶端執行多個重試，和調查原因，如果這種情況。 若要判斷用戶端是否正在執行多個重試，您可以︰
...
如果您看到的**PercentThrottlingError**值與高活動的應用程式的期間一致的波峰上，您應該在您的用戶端中實作關閉策略重試指數 （不線性） 上的一步︰ 這會減少磁碟分割上的立即載入，協助您的應用程式平滑出尖峰流量。 如需有關如何實作重試原則使用儲存空間的用戶端文件庫的詳細資訊，請參閱[Microsoft.WindowsAzure.Storage.RetryPolicies 命名空間](http://msdn.microsoft.com/library/azure/microsoft.windowsazure.storage.retrypolicies.aspx)。
...
- 會在 SA 機碼中的版本參數 (例如**sv = 2015年-04-05**) 符合您所使用的儲存空間用戶端文件庫的版本嗎？ 我們建議您永遠使用最新版的[儲存空間用戶端文件庫](https://www.nuget.org/packages/WindowsAzure.Storage/)。
- 如果您重新產生您儲存便捷鍵，這可能造成任何現有的 SA 權杖。 如果您產生 SA 權杖長的到期時間快取的用戶端應用程式，這可能是問題。

如果您使用儲存空間的用戶端文件庫產生 SA 權杖，然後您可以輕鬆建立的有效的權杖。 不過，如果您是使用儲存空間 REST API，並以手動方式建構 SA 權杖應該仔細閱讀主題[委派存取共用 Access 簽章](http://msdn.microsoft.com/library/azure/ee395415.aspx)。
...
許多工具可讓您從 Azure 資料表儲存體，方便您將資料載入到 Excel，供檢視及分析的分隔格式下載儲存指標資料。 從 Azure blob 儲存體儲存記錄資料位於您可以在 Excel 中載入分隔格式。 不過，您必須新增適當的欄標題的[儲存空間分析記錄格式](http://msdn.microsoft.com/library/azure/hh343259.aspx)，[儲存分析指標表格的結構描述](http://msdn.microsoft.com/library/azure/hh343264.aspx)的資訊。
